...
const AddProduct = () => {
  ...
  const handleChange = (
    ...
    setFormData((prev) => {
      ...
      // Sync with first variant if it's the only one
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      // Create API payload
      // We can send everything as FormData, but arrays are tricky in FormData.
      // Usually, we append indexed keys like variants[0][name].
      // Or we can send JSON string for variants if the backend parses it.
      // Looking at the controller logic, check 'req.body.variants' logic.
      // Express 'multer' handles fields, but nested objects usually need flattening or JSONification.
      // Let's assume standard FormData appending for now, or JSON stringify if object.
      
      // NOTE: Axios handles FormData well, but backend multer middleware might separate text fields.
      // Let's rely on standard practice: append keys.
      ...
      // Handle variants - sending as JSON string is safest if backend parses it manualy, 
      // but standard express/multer usually parses x[0][y] syntax if configured extended.
      // HOWEVER, 'req.body.variants' in backend is expected to be an array object.
      // The safest way with file upload (multipart) is often to send complex data as a JSON string key.
      // But looking at previous code, let's try standard append first.
      
      // Actually, to be safe with most configurations:
      // Handle variants - Sending as JSON string is most reliable with FormData
      // Handle variants - sending as JSON string
      ...
      >
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <input
                ...
                placeholder="Enter product name (e.g. Quaker Oats)"
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">
                Main Product Image (Thumbnail) <span className="text-red-500">*</span>
              </label>
              <input
                type="file"
                name="image"
              ...
              {preview && (
                <div className="mt-4">
                  ...
                  <img
                    src={preview}
                    alt="Main Preview"
                    className="w-full max-w-xs h-40 object-cover rounded-lg border border-gray-200 shadow-sm"
                  />
                </div>
              )}
            </div>
                ...
                <div className="space-y-1 text-center">
                  ...
                  <div className="flex text-sm text-gray-600 justify-center">
                    <label
                      ...
                      className="relative cursor-pointer bg-white rounded-md font-medium text-indigo-600 hover:text-indigo-500 focus-within:outline-none px-1"
                    >
                  ...
                  <p className="text-xs text-gray-500">Add up to 10 more images</p>
                </div>
              </div>
            ...
            {/* BASE PRICE & STOCK (Legacy / Default) */}
                ...
                >
                  ...
                  <div className="space-y-4 max-h-[600px] overflow-y-auto pr-2">
                    {formData.variants?.map((variant, index) => {
                       // Hide if it's the only one and has internal name
                       ...
                       return (
                        <div
                          key={index}
                          className="p-4 bg-gray-50/50 border border-gray-200 rounded-xl relative group hover:border-indigo-200 transition-colors"
                        >
                    <button
                      type="button"
        ...
        <div className="mt-8 flex gap-4 pt-6 border-t border-gray-100">
          ...
          <button
            type="submit"
            disabled={loading}
            className="flex-1 flex items-center justify-center gap-2 bg-indigo-600 text-white px-6 py-3 rounded-lg hover:bg-indigo-700 transition-all font-semibold disabled:opacity-50 disabled:cursor-not-allowed shadow-lg hover:shadow-xl"
          >
